...
export const useInitialSectionScroll = () => {
  ...
  const computeScrollTopToAnchor = (
  ...
  const syncSpacerForSection = (section: BottomSheetFilterInitialSection) => {
    const scroller = contentFrameRef.current;
  ...
  const scrollToInitialSection = (section: BottomSheetFilterInitialSection) => {
    const scroller = contentFrameRef.current;
    if (!scroller) return;
